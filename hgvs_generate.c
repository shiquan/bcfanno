...
struct hgvs_names_description {
    ...
    int32_t end;
    /* for var_type_snp ref_length ==1, for var_type_dels ref_length = length(var_type_dels), 
...
void generate_hgvs_names(
    struct gene_predictions_memory_pool *pool,
    bcf1_t *line,
    int *n_names1, char **names1,
    int *n_hgvs, char **hgvs_names,
    int *n_names2, char **names2
    )
{
    *n_names1 = 0;
    *n_names2 = 0;
    *n_hgvs = line->n_allele -1;

    if (*n_hgvs == 0) return;
    bcf_dec_t *d = &line->d;
    hgvs_names = (char**)calloc(*n_hgvs, sizeof(char*));
...
static void setter_hgvs_string(bcf_hdr_t *hdr, bcf1_t *line, const char *key, int n, char **names)
{
}
static void setter_gene_name_string(bcf_hdr_t *hdr, bcf1_t *line, const char *key, int n, char **names)
{
}
static void setter_transcript_string(bcf_hdr_t *hdr, bcf1_t *line, const char *key, int n, char **names)
{
}
...
void anno_hgvs_core(struct gene_predictions_memory_pool *pool, htsFile *fp, tbx_t *tbx, bcf_hdr_t *hdr, bcf1_t *line)
{
    ...
    if (pool->l == 0) return;
    const char *hgvs_key = "HGVS.name";
    const char *name_key = "GENE";
    const char *transcript_key = "Transcript";
